...
module F32_ba : PixmapType = struct
  ...
  type t = (float, Bigarray.float32_elt, Bigarray.c_layout) Bigarray.Array1.t * int * int
...
(** [Stroker] contains all the implementation of algorithms needed to render 2D graphics
    primitives shuch as lines or Bézier curves.

    All point coordinates used by the following functions are assumed to be scaled (see
    {!state.scaling}). ) *)
module Stroker = struct
  (** [bresenham_line x0 y0 x1 y1 w] computes points of the line from ([x0], [y0]) to
      ([x1], [y1]) with a width [w].

      It uses the Bresenham's line algorithm:
      https://en.wikipedia.org/wiki/Bresenham's_line_algorithm

      ISSUE: Stack_overflow for a certain [w] value. *)
  let bresenham_line (x0 : int) (y0 : int) (x1 : int) (y1 : int) (w : int) : p2 list =
    let dx = abs (x1 - x0)
    ...
    let width_line x y =
      (* Add width support to the Bresenham's algorithm. *)
      let calculate (x : int) (y : int) : p2 list =
        ...
        let rec loop acc x =
          let acc =
            (if is_horizontal
            then P2.v (float_of_int y) (float_of_int x)
  ...
  (** [cubic_bezier ?nb_line p1x p1y c1x c1y c2x c2y p2x p2y] returns all the coordinates
      of lines approaching the Bézier curve.
  ...
  (** [cubic_bezier ?nb_line p1x p1y c1x c1y p2x p2y] returns all the coordinates of lines
      approaching the Bézier curve.
  ...
  let quadratic_bezier
    ...
    let rec loop acc t =
      ...
      then acc
        ...
        and c = t' ** 2. in
        let x, y =
          (a *. p1x) +. (b *. c1x) +. (c *. p2x), (a *. p1y) +. (b *. c1y) +. (c *. p2y)
        in
        loop (P2.v x y :: acc) (t +. 1.))
    in
    loop [] 0.
  ...
  (** [arc_to_bezier px py cx cy rx ry angle large sweep] returns all the coordinates of
      lines approaching the curve using cubic Bézier curves. *)
  let arc_to_bezier
      (px : float)
      (py : float)
      (cx : float)
    ...
    let approx_unit_arc a1 a2 =
      let c1 = 1.570_796_326_794_896_6
      and c2 = 0.551_915_024_494 in
      let a =
        if c1 = a2
        then c2
        else if -.c1 = a2
      ...
      (x1 -. (y1 *. a), y1 +. (x1 *. a)), (x2 +. (y2 *. a), y2 -. (x2 *. a)), (x2, y2)
    in
    ...
    then []
      ...
      and pyp = (-.sinphi *. (px -. cx) /. 2.) +. (cosphi *. (py -. cy) /. 2.) in
      if 0. = pxp && 0. = pyp
      then []
      else (
        let rx = Float.abs rx
        and ry = Float.abs ry in
        let lambda = Float.((pow pxp 2. /. pow rx 2.) +. (pow pyp 2. /. pow ry 2.)) in
        let rx, ry =
          if 1. < lambda
          then (
...
module Filler = struct
  ...
  (** [update_tables y et aet] updates the edge table and the active edge table according
      the given current [y]-value. *)
  let update_tables (y : float) (et : edge list) (aet : edge list) : edge list * edge list
    =
    let to_move = ref [] in
    let et =
      et
  ...
  (** [scanline r f poly] is an implementation of the scanline rendering algorithm. It
      apply the [f] function to each points of each crossing lines according the given
      rule [r]. *)
  let scanline (r : [ `Anz | `Aeo ]) (f : float -> float -> unit) (poly : p2 list list)
      : unit
    =
    let rec scan ?(start = false) y aet et =
      (* Iterates over each lines of the path. *)
      if start || 0 <> List.length aet || 0 <> List.length et
      then (
        ...
        let winding_nb = ref (List.fold_left (fun acc e -> acc + e.direction) 0 aet) in
        let aet =
          aet
          |> List.mapi (fun i e0 ->
                 ...
                 then (
                   ...
                   for i = int_of_float e0.xstart + 1 to int_of_float e1.xstart do
                     f (float_of_int i) y
                   done);
...
module Make (Pixmap : PixmapType) = struct
  ...
  let to_int_coords ((x, y) : float * float) : int * int = int_of_float x, int_of_float y

  let to_float_coords ((x, y) : int * int) : float * float =
    float_of_int x, float_of_int y
  ;;

  let get_curr_int_coords (s : state) : int * int = to_int_coords P2.(x s.curr, y s.curr)

  (** [get_real_coords s x y] returns the corresponding coordinate of the normalized ([x],
      [y]). *)
  ...
  (** [move_to s pt] updates the current position to ([pt.x], [pt.y]) and opan a new
      sub-path starting at ([pt.x], [pt.y]). *)
  let move_to (s : state) (pt : p2) : unit =
    s.curr <- pt;
    s.path <- { empty_subpath with start = Some s.curr } :: s.path
  ;;

  (** [add_path_points s pts] adds [pts] to the current path and if it's empty, begins a
      new one starting at [s.curr]. *)
  ...
  (** [line_to s] pt adds a line to the path from the current point to position ([pt.x],
      [pt.y]) scaled by [s.scaling]. After this call the current point will be ([pt.x],
      [pt.y]). *)
  ...
  (** [close_path s] adds a line segment to the current path being built from the current
      point to the beginning of the current sub-path before closing it. After this call
      the current point will be at the joined endpoint of the sub-path.

      If there is no current point before the call to [close_path], this function will
      have no effect. *)
  ...
  (** [bezier_curve_to s t c1x c1y c2y ptx pty] adds points to the current [s.path]
      according the given bezier curve type [t]*)
  let bezier_curve_to
      (s : state)
      (t : [< `Quad | `Cubic ])
      (c1x : float)
  ...
  let set_path (s : state) (p : Pv.Data.path) : unit =
    ...
    |> List.iter (function
           ...
           | `Ccurve (c, c', pt) ->
             bezier_curve_to s `Cubic (x c) (y c) ~c2x:(x c') ~c2y:(y c') (x pt) (y pt)
           | `Earc (large, cw, r, a, pt) ->
             ...
             | Some (c, m, a, a') ->
               ...
               (* FIXME: *)
               Stroker.arc_to_bezier (x s.curr) (y s.curr) (x c) (y c) a a' 0. cw large
  ...
  (** [is_in_view s x y] for now, verifies that (x, y) are valid coordinates for the
      [s.pixmap]. TODO: need to find out how to manage the [view] and the [size]. *)
  let is_in_view (s : state) (x : float) (y : float) : bool =
    let w, h = to_float_coords B.(w s.pixmap, h s.pixmap) in
    x >= 0. && x < w && y >= 0. && y < h
  ;;

  (** [r_path s] returns all points of the current path (which must only be combosed of
      lines. *)
      ...
      let _, _, pts =
        ...
        |> List.fold_left
             (fun (i, prev, pts) pt ->
               ...
               let width, _ =
                 if s.use_width
                 then get_real_coords s (s.gstate.g_outline.width, 1.) |> to_int_coords
                 else 1, 1
               in
               s.curr <- P2.v x1 y1;
               i + 1, P2.v x1 y1, Stroker.bresenham_line x0 y0 x1' y1' width @ pts)
             (0, start, [])
      in
      pts
    in
  ...
  let r_stroke (s : state) : unit = r_path s |> List.iter (draw_point s s.gstate.g_stroke)

  (** [push_transform s tr] updates the current transformation matrix by applying the
      transformation [tr]. *)
  ...
  (** [r_fill r s] fills all the points inside [s.path] according to the given filling
      rule [r].
  ...
  let create_state
    ...
    ; todo = []
    ; gstate =
        { g_tr = init_tr; g_outline = P.o; g_stroke = Color.void; g_fill = Color.void }
    }
  ;;
  ...
  let render_target
    ...
    let render v k r =
      ...
      | `End -> k r
      | `Image (size, view, i) ->
        let s = create_state pixmap res size view use_width r in
        s.todo <- [ Draw i ];
        r_image s k r
    in
    false, render
  ...
  let target pixmap res ?(use_width = false) =
    Pv.create_target (render_target pixmap res use_width)
  ;;
end
